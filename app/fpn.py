...
def feature_extraction(session_state):
    ...
    """
    st.title("Multi-scale Feature Extraction")
    st.write(
        ...
        be augmented to maintain scale invariant representations because in the real-world, objects from the same class can exist at a wide range of sizes depending on their depth in an image. \
        ...
        levels from a [ResNet](https://arxiv.org/pdf/1512.03385.pdf) backbone."
    )
    ...
    with st.beta_expander("How Do FPNs Work?", expanded=False):
        ...
        st.write(
            "**a. Bottom-up Pathway:** An FPN can be constructed from any deep CNN, but RetinaNet chooses a ResNet architecture. In ResNet, convolutional layers are grouped together \
                ...
                RetinaNet constructs a pyramid with levels P$_3$ - P$_7$, where P$_l$ indicates pyramid level and has reslution 2$^l$ lower than the input. P$_0$ - P$_2$ are omitted from the FPN \
                due to the high dimensional impact on memory and computation."
        )
        st.write(
